...
wordsArray = database_handler.get_data_from_database('words', ['words_array'])
DELAY_TIME = 3
...
async def get_random_word(msg, args):
    ...
    if len(wordsArray) == 0:
        await msg.channel.send(f'{msg.author.mention}, я пока не знаю никаких слов, '
                               f'вы можете добавить их в меня через команду '
                               f'или вручную через менеджера баз данных', delete_after=DELAY_TIME)
        ...
        await msg.channel.send(f'{current_user} {random.choice(wordsArray)}')


def check_for_spam(msg):
    current_status = database_handler.get_data_from_database('variables',
                                                             ['spammerID', 'spammerCount'])
    if current_status[0] == msg.author.id:
        if current_status[1] >= 3:
            database_handler.edit_data_in_database('variables',
                                                   ['spammerCount'],
                                                   [0])
            return True
        database_handler.edit_data_in_database('variables',
